...
gulp.task('templates', function () {
    gulp.src(config.jade).pipe(changed(config.jade)).pipe(jade({
        doctype: 'html',
        pretty: false
...
gulp.task('styles', function () {
    return gulp.src(config.sass).pipe(changed(config.distCss)).pipe(compass({
...
gulp.task('images', function () {
    return gulp.src([config.image, '!images/icons/*']).pipe(changed(config.image)).pipe(minimage()).pipe(gulp.dest(config.distImg)).pipe(reload({ stream: true }));
});
...
gulp.task('watchify', function () {
    ...
    function bundle() {
        return b.bundle()
        // 如果有错误发生，记录这些错误
        .on('error', gutil.log.bind(gutil, 'Browserify Error')).pipe(source(config.mainJs))
        ...
        .pipe(gulp.dest(config.distScript)).pipe(reload({ stream: true }));
    }
...
gulp.task('browserify', function () {
    return browserify(config.js).transform(babelify).bundle().pipe(source(config.mainJs)).pipe(buffer()).pipe(sourcemaps.init({ loadMaps: true })).pipe(uglify()).pipe(sourcemaps.write('./')).pipe(gulp.dest(config.distScript)).pipe(reload({ stream: true }));
});
